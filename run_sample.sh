...
# Help message
usage() {
    echo "Usage: bash run_sample.sh [options]"
    echo ""
    echo "Options:"
    echo "  --name <str>      Sample name"
    ...
    echo "  --l <int>         Total reported target length, centered on target (optional)"
    echo "  --res <int>       Number of top allele/reads to export (default: 5)"
...
if [ -n "$TARGET_LEN" ]; then
    if ! [[ "$TARGET_LEN" =~ ^[0-9]+$ ]] || [ "$TARGET_LEN" -lt 1 ]; then
        echo "Error: --l must be a positive integer."
        exit 1
    fi
fi

if ! [[ "$RES" =~ ^[0-9]+$ ]] || [ "$RES" -lt 1 ]; then
...
echo "Reported target length (--l): ${TARGET_LEN:-auto}"
echo "Top reads to export (--res): $RES"
...
if [ ! -d "$OUTPUT_DIR/BAM" ] || [ -z "$(find "$OUTPUT_DIR/BAM" -maxdepth 1 -type f -name "*.sorted.bam" | head -n 1)" ]; then
