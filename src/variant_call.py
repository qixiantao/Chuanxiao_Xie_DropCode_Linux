...
def determine_target_interval_from_metadata(
    ...
    if orientation == "forward":
        ...
        interval_end = target_start + extract_len

    elif orientation in {"reverse", "reverse_complement"}:
        interval_end = target_end
        interval_start = target_end - extract_len
        orientation = "reverse"
    ...
    logger.info(
        "Using target metadata: chrom=%s, interval=%d-%d, orientation=%s",
        chrom,
        interval_start + 1,
        interval_end,
        orientation
...
def determine_target_interval_from_anchor(
    ...
    if orientation == "forward":
        ...
        interval_end = interval_start + extract_len
    else:
        interval_end = position
        interval_start = interval_end - extract_len
...
def write_results_excel(results, output_xlsx):
    ...
    columns = [
        ...
        "Allele1_Seq_Count_Percent_Type",
        "Allele2_Seq_Count_Percent_Type",
        "Allele3_Seq_Count_Percent_Type",
        "Allele4_Seq_Count_Percent_Type",
        "Allele5_Seq_Count_Percent_Type"
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Length of the wild-type target interval; default: 20"
    )
    parser.add_argument(
        "--f",
    ...
    try:
        ...
        if args.extract_len < 1:
            raise ValueError("--extract_len must be a positive integer.")
        ...
        logger.info("Wild-type allele: %s", wildtype_allele)
        ...
        for bam_path in bam_files:
            ...
            if not alleles:
                ...
                top_display = recalculated_items[:5]
